    ...
    B: 'static,
{
    ...
    fn call(&self, request: ServiceRequest) -> Self::Future {
        let service_context_service = request.app_data::<web::Data<dyn ContextService>>().unwrap();

        if should_display_maintenance(service_context_service, &request) {
...
fn should_display_maintenance(
    service_context_service: &web::Data<dyn ContextService>,
    request: &ServiceRequest,
) -> bool {
    service_context_service.is_maintenance_active() && !request.path().starts_with("/context")
}
